   ...
   return {
       ...
       controller: ['$scope', '$rootScope', '$document', '$element', 'service', 'course', '$timeout', 'appraiseService', '$filter', function ($scope, $rootScope, $document, $element, service, course, $timeout, appraiseService, $filter) {
           ...
           function init() {
               ...
               if ($scope.autoInit) {
                   if (!$scope.targetId) {
                       console.error('[appraise-list]: 课程信息有误，无法获取评价');
                   ...
                   $scope.$watch('data', function(newValue, oldValue) {
                       if (!newValue) {
                           console.log('[appraise-list]: 传入的评价数据为空');
                           return;
                       }
                       if (!newValue.valid) {
                           console.error('[appraise-list]: 传入的评价数据无效', newValue);
                           $scope.cmtResult = appraiseService.getBasicTargetComment();
                       } else {
                           $scope.cmtResult = newValue;
                           $scope.pagefnCallback({pa: {total: newValue.total}});
                           console.log('[appraise-list]: 传入的评价数据更新', newValue);
                       }
                       recalculate();
                   });
               }
           ...
           function pagefn(pageargs, callback) {
               ...
               commonLoaderTimer = $timeout(function () {
                  $scope.commentLoaded = false;
               }, 800);
           ...
           function loadComment(params, callback) {
               appraiseService.getTargetComment(params, function (ok, data) {
                   console.log('[appraise-list]: loadComment', ok, data);
                   if (ok) {
           ...
           function recalculate() {
               ...
               if (commonLoaderTimer) {
                   $timeout.cancel(commonLoaderTimer);
                   commonLoaderTimer = null;
               }
           ...
           $scope.reloadList = function (index) {
               ...
               console.log('[appraise-list]: reloadList............')
           };
