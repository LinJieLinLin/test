...
module.controller('contentCtr', ['$rootScope', '$scope', '$timeout', '$interval', 'service', 'cvfa', function($rootScope, $scope, $timeout, $interval, service, cvfa) {
    //停用vconsole
    var three = 1;
    ...
    $scope.showNewContentView = function() {
        ...
        if (!$scope.targetId) {
            ...
            cvfa.getLearnRecord([$rootScope.currentUser.uid + '_' + $scope.courseId], function(rs) {
                console.log($rootScope.currentUser.uid + '_' + $scope.courseId, '获取到记录：', rs);
                $scope.endingLoading();
    ...
    $scope.setViewPage = function(argPage) {
        ...
        console.log('记录预览页', argPage);
        cvfa.setKeyValue(cvfa.nowViewKey, argPage, 'viewDoc');
    };
    $scope.init();
