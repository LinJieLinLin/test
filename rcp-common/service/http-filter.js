        ...
        var responseInterceptor = {
            request: function(config) {
                var defaultOption = {          // 请求配置对象
                     ...
                     alertError: DYCONFIG.dev  // 是否提示code非0的返回
                };
                ...
                if (windowLocationHost.indexOf(':') > -1) {
                    windowLocationHost = 'rcp.dev.gdy.io';
                } else if (windowLocationHost.indexOf('sso') > -1) {
                    windowLocationHost = '*';
                }

                config.params = config.params || {};
                config.params.host = !config.params.host || config.params.host.indexOf('localhost') > -1 ? windowLocationHost : config.params.host;
                config.params.source = config.params.source || 'PC';
                return config;
            },
