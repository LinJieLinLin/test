...
module.factory("dynamicService", function() {
    var factory = {
        emotions: '表情服务'
            // images : '图片服务',
            // video : '视频服务',
...
module.directive("addface", function() {
    return {
        restrict: 'A',
        scope: {
            //同时在标签加上 bindeditor="#标签ID" 一起使用
            data: '=addface'
        },
...
module.filter("filterDynamicHtml", ['$sce', 'dynamicService', function($sce, dynamicService) {
    return function(htmlCode) {
        return $sce.trustAsHtml(dynamicService.emotions.AnalyticEmotion(htmlCode));
    };
}]);
module.filter("bindHtml", ['$sce', function($sce) {
    return function(htmlCode) {
