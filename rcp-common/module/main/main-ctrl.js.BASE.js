...
module.controller('mainCtrl', ['$scope', '$rootScope', '$http', 'service', '$cookies', '$timeout', function($scope, $rootScope, $http, service, $cookies, $timeout) {
    ...
    function loginSuCb(rs) {
        ...
        setNavMenu($rootScope.currentUser.role);
    ...
    /**
     * [setNavMenu 设置用户菜单]
     * @param {[type]} userTypes [description]
     */
    function setNavMenu(userTypes) {
        $scope.itemList = [];
        //增加一个item
            ...
            angular.forEach(arguments, function(attr) {
                switch (attr) {
                    ...
                    case '助学中心':
                        ...
                        break;
                        // case '老师认证':
                        //     var p = $scope.user.auth === -1 ? 1 :
                        //         $scope.user.auth === -2 ? 2 : '';
                        //     arr.push({ ic: 'L2', name: '教学中心', href: getUrl(attr, { p: p }) });
                        //     break;
                        // case '家长空间':
                        //     arr.push({ ic: 'L2', name: attr, href: getUrl(attr, { uid: $rootScope.currentUser.uid }) });
                        //     break;
                        // case '机构空间':
                        //     arr.push({ ic: 'L3', name: attr, href: getUrl(attr, { oid: $scope.user.orgId }) });
                        //     break;
                        // case '学校空间':
                        //     arr.push({ ic: 'L3', name: attr, href: getUrl(attr, { oid: $scope.user.schoolId }) });
                        //     break;
                        // case '班级空间':
                        //     arr.push({ ic: 'L3', name: attr, href: getUrl(attr, { oid: $scope.user.classId }) });
                        //     break;
                    case '管理中心':
                        arr.push({ ic: 'L1', name: attr, href: getUrl(attr) });
                        break;
                        // case '审核管理':
                        //     arr.push({ ic: 'L1', name: attr, href: getUrl(attr, '#/course-approval') });
                        //     break;
                        // case '人员管理':
                        //     arr.push({ ic: 'L1', name: attr, href: getUrl(attr) });
                        //     break;
                    case '创建新课程':
                        arr.push({ ic: 'L4', name: attr, href: getUrl('创建课程', { type: 10 }) });
                        break;
                        // case '创建新题库':
                        //     arr.push({ ic: 'L5', name: attr, href: getUrl('创建题库', { type: 20 }) });
                        //     break;
                }
            });
            return arr;
        }
        angular.forEach(userTypes, function(v, k) {
            switch (k) {
                case 'default':
                    break;
                case 'student':
                ...
                case 'school':
                    ...
                    //new
                    // case 'UCS_ADMIN':
                    //     addNavItem(setNavItem('审核管理', '人员管理'));
                    //     break;
        ...
        addNavItem(setNavItem('学习中心', '教学中心', '创建新课程', '创建新题库'));
    }
    /**
