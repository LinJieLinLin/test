...
pkgs="\
 com.dy.wcms/artapi\
"
ig_cpkgs="com.dy.wcms/artapi"
ig_pkg="com.dy.wcms/srv"
ig_n="srv.test"
main_n="ars"
# main_pkg="com.dy.wcms/main"
set -e
ftp_arg="-a"
u_n=`uname`
case $u_n in
...
build_main(){
	echo "Build Main"
	go build -o $WS_B_DIR/bin/$main_n www/test/web/ars.go
}
##############################
...
instrument(){
  ...
  # ln -s $PWD/../lib/jsup.js $PWD/www/js
	istanbul instrument --prefix $PWD/www --output $WS_B_DIR/www -x lib/** -x test/** -x bower_components/** $PWD/www
